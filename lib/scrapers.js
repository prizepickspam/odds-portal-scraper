 ...
 */
async function* extractOddsData(page, leagueName, limit) {
    ...
    const linksToProcess = limit ? uniqueLinks.slice(0, limit) : uniqueLinks;
...
async function extractMlOdds(page, oddTypeChoosen) {
    ...
    // logger.info("jack -- waiting for odd container selector")
    // setTimeout(() => {  console.log('World!'); }, 800);
    await page.waitForSelector('div[data-testid="odd-container-default"]');
    // logger.info("jack -- we found the odd container selector")
    ...
    return await page.$$eval(rowsSelector, rows =>
        rows.map(row => {
            ...
            const oddsContainers = row.querySelectorAll('div[data-testid="odd-container-default"]');
...
*/
async function* scrapeOdds(page, oddsFormat, leagueName, limit) {
    await setOddsFormat(page, oddsFormat)
    // logger.info("this is jack here")
    try {
        // await extractOddsData(page, callback, leagueName)
        for await (const item of extractOddsData(page, leagueName, limit)) {
            yield item;
        }
...
async function* scrapeOddsHistoric(page, oddsFormat, leagueName) {
    ...
    const firstNumbers = await page.locator("a.pagination-link").evaluateAll(els => els.map(el => el.textContent).filter(text => text !== 'Next'));

    if (['mls', 'championship'].includes(leagueName)){
        firstNumbers.push("11");

        if (leagueName === 'championship'){
            firstNumbers.push("12");
        }
    }
    const currentUrl = page.url();
    const pageNumbers = [...new Set(firstNumbers)].map(Number).sort((a, b) => a - b).map(String);
