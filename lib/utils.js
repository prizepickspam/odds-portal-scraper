...
export function getHistoricUrls(leagueName, startYear, endYear) {
    ...
    let leagueInfo = leaguesUrlsMap[leagueName];
    if (!leagueInfo) {
        throw new Error(`League '${leagueName}' is not referenced`);
    }

    let { url, fixedStructure } = leagueInfo;

    const years = Array.from({ length: parseInt(endYear) - parseInt(startYear) + 1 }, (_, i) => parseInt(startYear) + i);
    return years.map(year => {

        if (['brazil-serie-a', 'argentina'].includes(leagueName) && [2024, 2025].includes(parseInt(year))) {
            leagueInfo = leaguesUrlsMap[`${leagueName}-new`];
            url = leagueInfo.url;
            fixedStructure = leagueInfo.fixedStructure;
        }

        if (parseInt(year) === 2025) {
            // For 2025, return url/results/ regardless of league structure
            return `${url}/results/`;
        }
        return fixedStructure
            ? `${url}-${year}/results/`
            : `${url}-${year}-${parseInt(year) + 1}/results/`;
    });
}
